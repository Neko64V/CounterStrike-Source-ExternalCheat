...
bool Overlay::CreateOverlay()
{
    ...
    RegisterClassExA(&wc);
    m_hHwnd = CreateWindowExA(m_lDefaultFlag, wc.lpszClassName, wc.lpszMenuName, WS_POPUP | WS_VISIBLE, g.g_GamePoint.x, g.g_GamePoint.y, g.g_GameRect.right, g.g_GameRect.bottom, NULL, NULL, wc.hInstance, NULL);
    SetLayeredWindowAttributes(m_hHwnd, RGB(0, 0, 0), 255, LWA_ALPHA);
    MARGINS margin = { -1 };
    DwmExtendFrameIntoClientArea(m_hHwnd, &margin);

    if (!CreateDeviceD3D(m_hHwnd))
    {
        CleanupDeviceD3D();
